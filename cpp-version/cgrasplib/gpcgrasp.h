...
#include "gpcg/gpcg_constructGreedyRand.h"
#include "gpcg/gpcg_localImprove.h"
...
Vld CGrasp(Ld function(Vld V), Vld &L, Vld &U,
	...
	#pragma omp target teams distribute parallel for num_threads(num_threads_cgrasp)
	...
	for(int iter = 0; iter<max_iterations; iter++){
		...
		{
			if(iterMelhor>0 && ObjF > objective)
				loop = 1;
		}
